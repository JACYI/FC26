// ==UserScript==
// @name         【FSU】EAFC FUT WEB 增强器 - JACYI C版
// @namespace    https://github.com/JACYI
// @version      26.10-jacyi.1
// @description  【JACYI优化版】基于原版FSU增强器优化改进。EAFCFUT模式SBC任务便捷操作增强器，模拟开包、额外信息展示、近期低价自动查询、一键挂出球员、跳转FUTBIN、快捷搜索、拍卖行优化等
// @author       JACYI
...
// ==/UserScript==
/* ===========================================================
   【FSU】EAFC FUT WEB 增强器 - JACYI优化版
   ===========================================================
   原版作者: Futcd_kcka (https://greasyfork.org/scripts/431044)
   优化作者: JACYI
   版本:     26.10-jacyi.1
   C版说明:  集大成者 — 基于 jackyi.1（B脚本）合并 Fodder（A脚本）精华：
             批量开包引擎重构、可交易包全出售、AUTO SOLVE 双算法、自定义流程
   说明:     本脚本基于原版 FSU 增强器进行优化和改进，
             版权所有归原版作者所有。
             Modified and optimized by @JACYI.
   =========================================================== */
...
(function () {
    ...
    // ===== [C] 防重入守卫：与 B 脚本（fsu-mod.jackyi.js）并存时只运行一个 =====
    // 启用 C 必须禁用 B（两者都改 EA 原型，双跑必然互踩）
    if (typeof unsafeWindow !== 'undefined' && unsafeWindow.__fsuJacyiC) { return; }
    if (typeof unsafeWindow !== 'undefined') { unsafeWindow.__fsuJacyiC = true; }
    ...
    function futweb() {
        ...
        info = {
            ...
            "setfield":{"card":["pos","price","other","club","low","meta"],"player":["auction","futbin","getprice","loas","uatoclub","transfertoclub","pickbest"],"sbc":["top","right","quick","duplicate","records","input","icount","market","sback","cback","smartfill","conceptbuy","meetsreq","headentrance"],"info":["obj","sbc","sbcf","sbcs","pack","squad","skipanimation","sbcagain","packagain","discardbs"]},
            "set":{},
            "lock":[],
        ...
        //26.09-mod-04 防重入锁，防止短时间内多次触发 init 导致请求风暴
        events._initLock = false;
        events.init =  async function(){
            if (events._initLock) {
                console.log("[FSU] init already in progress, skipping");
                return;
            }
            events._initLock = true;
            try {
            SBCCount.init();
            set.init();
            build.init();
            lock.init();
            futbinId.init();
            //26.09-mod-06 从GM storage加载缓存的fastsbc数据
            var _cachedFastsbc = JSON.parse(GM_getValue("fastsbc", "null"));
            if (_cachedFastsbc) { _.merge(info.base.fastsbc, _cachedFastsbc); }
                        ...
                        if(_.size(data["api"])){
                            ...
                            if(_.has(info.api,"meta")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] meta API error:", err); }
                                })
                            }
                            if(_.has(info.api,"fastsbc")){
                                GM_xmlhttpRequest({
                                    ...
                                    onload: function(res) {
                                        ...
                                        //26.09-mod-06 持久化到GM storage
                                        GM_setValue("fastsbc", JSON.stringify(info.base.fastsbc));
                                    },
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){
                                        console.warn("[FSU] fastsbc API error:", err);
                                        //26.09-mod-06 服务器挂了则从缓存加载
                                        var _cached = JSON.parse(GM_getValue("fastsbc", "null"));
                                        if (_cached) { info.base.fastsbc = _cached; }
                                    }
                                });
                            ...
                            if(_.has(info.api,"pack")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] pack API error:", err); }
                                });
                            ...
                            if(_.has(info.api,"sbc")){
                                GM_xmlhttpRequest({
                                    ...
                                    onload:function(res){
                                        ...
                                        //26.09-mod-05 缓存SBC数据到本地，API挂时使用上次缓存
                                        GM_setValue("sbc_stat", JSON.stringify(sbcJson));
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){
                                        console.warn("[FSU] sbc API error:", err);
                                        //26.09-mod-05 API挂时使用上次缓存
                                        var _cached = JSON.parse(GM_getValue("sbc_stat", "null"));
                                        if (_cached) { info.task.sbc.stat = _cached; }
                                    }
                                });
                            }
                            if(_.has(info.api,"ggrating")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] ggrating API error:", err); }
                                })
                            }
                            //26.02 加载新进化信息
                            if(_.has(info.api,"evolutions")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] evolutions API error:", err); }
                                })
                            }
                            //26.04 加载包内球员ids
                            if(_.has(info.api,"inpacks")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] inpacks API error:", err); }
                                })
                            }
                            //26.04 加载其他配置
                            if(_.has(info.api,"other")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] other API error:", err); }
                                })
                            }
                            ...
                            if(_.has(info.api,"fgconfig")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] fgconfig API error:", err); }
                                })
                            }
                            ...
                            if(_.has(info.api,"playermeta")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] playermeta API error:", err); }
                                })
                            }
                            ...
                            if(_.has(info.api,"lowprice")){
                                GM_xmlhttpRequest({
                                    ...
                                    //26.09-mod-04 静默处理外部API失败
                                    onerror:function(err){ console.warn("[FSU] lowprice API error:", err); }
                                })
                            }
                        }
                    }
        ...
        events.hideLoader = () => {
            ...
            if(info.run.template){
                ...
                //26.10-jacyi.8 [C-06] fillSquadBtn 已并入智能填充 → 恢复 smartFillBtn 交互态
                if(isPhone()){
                    if(cntlr.current() instanceof UTSBCSquadOverviewViewController){
                        cntlr.current()._fsu?.smartFillBtn?.setInteractionState(1);
                        ...
                        _.forEach(cntlr.current().getNavigationController().childViewControllers, c => {
                            if(c instanceof UTSBCSquadOverviewViewController){
                                c._fsu?.smartFillBtn?.setInteractionState(1);
                            }
                        })
                    }
                }else{
                    cntlr.left()._fsu?.smartFillBtn?.setInteractionState(1);
                }
            }
        ...
        info.localization = {
            ...
            "openpack.storebtn.text":["批量开包","批量開包","Bulk Open"],
            ...
            "openpack.storebtn.popupt":["批量开包提示 - %1","批量開包提示 - %1","Bulk Open Notice - %1"],
            ...
            "openpack.progress.unassigned":["未分配 {count} 个","未分配 {count} 個","Unassigned {count}"],
            "openpack.progress.toclub":["发送俱乐部 {count} 个","發送到俱樂部 {count} 個","Sent {count} to Club"],
            "openpack.progress.tostorage":["放入SBC仓库 {count} 个","放入SBC倉庫 {count} 個","Sent {count} to SBC Storage"],
            "openpack.progress.tosell":["出售物品 {count} 个","出售物品 {count} 個","Sold {count}"],
            "openpack.progress.redeem":["兑换 {count} 个","兌換 {count} 個","Redeemed {count}"],
            "openpack.popup.untradable.warn":["⚠️ 该扩展包不可交易：出售重复/全部出售将直接兑换金币，无法上架转会市场！","⚠️ 該擴展包不可交易：出售重複/全部出售將直接兌換金幣，無法上架轉會市場！","⚠️ This pack is untradeable: Sell Dupes / Sell All convert items straight to coins — they cannot be listed on the Transfer Market!"],
            "openpack.result.popupm3":["共开启 %1 个球员包（%2个未开启），分配俱乐部 %3 个、SBC仓库 %4 个，出售 %5 个（+%6 金币），%7 个特别球员，最高评分 %8，未分配 %9 个，兑换 %10 个。","共開啟 %1 個球員包（尚有 %2 個未開啟），已分配至俱樂部 %3 個、SBC 倉庫 %4 個，出售 %5 個（+%6 金幣），%7 名特別球員，最高評分為 %8，未分配 %9 個，兌換 %10 個。","Opened %1 player packs (%2 not opened), assigned %3 to Club, %4 to SBC storage, sold %5 (+%6 coins), %7 special players, highest rating %8, %9 unassigned, %10 redeemed."],
            ...
            "smartfill.btntext":["智能填充","智能填充","Smart Fill"],
            "smartfill.popupt":["智能填充选项","智能填充選項","Smart Fill Options"],
            "smartfill.pool":["球员来源","球員來源","Player source"],
            "smartfill.pool.unassigned":["未分配仓库","未分配倉庫","Unassigned"],
            "smartfill.pool.storage":["队内仓库","隊內倉庫","Club & Storage"],
            "smartfill.option.untradeable":["仅不可交易","僅不可交易","Untradeable only"],
            "smartfill.option.dup":["优先重复","優先重複","Duplicates first"],
            "smartfill.option.commons":["仅普通卡","僅普通卡","Non-special only"],
            "smartfill.option.template":["复杂SBC优先Futbin模板","複雜SBC優先Futbin模板","Futbin templates first (complex SBCs)"],
            "smartfill.option.ref":["方案链接或ID（可选）","方案連結或ID（可選）","Template link / ID (optional)"],
            "smartfill.option.remember":["记住此SBC选项","記住此SBC選項","Remember for this SBC"],
            "smartfill.ratingrange":["评分范围","評分範圍","Rating range"],
            "smartfill.pricerange":["价格范围","價格範圍","Price range"],
            "smartfill.template.ok":["已应用Futbin方案","已應用Futbin方案","Futbin template applied"],
            "smartfill.template.fail":["Futbin方案不可用，改用本地算法","Futbin方案不可用，改用本地演算法","Template unavailable, falling back to local engine"],
            "set.sbc.smartfill":["智能填充","智能填充","Smart Fill"],
            "set.sbcFill.title":["智能填充默认选项","智能填充默認選項","Smart Fill defaults"],
            "set.sbcFill.minRating":["默认最低评分","默認最低評分","Default min rating"],
            "set.sbcFill.maxRating":["默认最高评分","默認最高評分","Default max rating"],
            "set.sbcFill.minPrice":["默认最低价格","默認最低價格","Default min price"],
            "set.sbcFill.maxPrice":["默认最高价格","默認最高價格","Default max price"],
            "set.sbcFill.preferDuplicates":["默认优先重复","默認優先重複","Prefer duplicates by default"],
            "set.sbcFill.untradeableOnly":["默认仅不可交易","默認僅不可交易","Untradeable only by default"],
            "set.sbcFill.commonsOnly":["默认仅普通卡","默認僅普通卡","Non-special only by default"],
            "set.sbcFill.useTemplate":["默认模板优先","默認模板優先","Templates first by default"],
            "set.sbcFill.poolSource":["默认球员来源","默認球員來源","Default player source"],
        ...
        UTPlayerItemView.prototype.renderItem = function (p, t) {
            ...
            if (p.isValid()) {
                setTimeout(() => {
                    ...
                    //26.09-mod-01 修复评分显示不准确：加载元数据后刷新 + 点击刷新
                    ...
                    //点击评分手动刷新元数据（修复评分显示不准确）
                    this._fsu.ratingBackground.addEventListener('click', function(e) {
                        e.stopPropagation();
                        var self = this;
                        services.PlayerMetaData.updateItemPlayerMeta([p]).observe(cntlr.current(), function(t, e) {
                            t.unobserve(cntlr.current());
                            if (self._fsu && self._fsu.ratingBackground && self.__root) {
                                self._fsu.ratingBackground.textContent = p.rating;
                            }
                        });
                    }.bind(this));
                    //元数据未加载时自动加载并刷新评分
                    if (!services.PlayerMetaData.metaDAO.metaRepo.has(p.definitionId)) {
                        var self = this;
                        services.PlayerMetaData.updateItemPlayerMeta([p]).observe(cntlr.current(), function(t, e) {
                            t.unobserve(cntlr.current());
                            if (self._fsu && self._fsu.ratingBackground && self.__root) {
        ...
        //26.09-mod-01 补充：Large/Small 球员卡加载元数据确保评分正确
        if (typeof UTLargePlayerItemView !== 'undefined' && UTLargePlayerItemView.prototype.hasOwnProperty('renderItem')) {
            const _origLargeRender = UTLargePlayerItemView.prototype.renderItem;
            UTLargePlayerItemView.prototype.renderItem = function(p, t, e) {
                _origLargeRender.call(this, p, t, e);
                if (p && p.type === ItemType.PLAYER && !services.PlayerMetaData.metaDAO.metaRepo.has(p.definitionId)) {
                    services.PlayerMetaData.updateItemPlayerMeta([p]).observe(cntlr.current(), function(t, e) {
                        t.unobserve(cntlr.current());
                    });
                }
        ...
        if (typeof UTSmallPlayerItemView !== 'undefined' && UTSmallPlayerItemView.prototype.hasOwnProperty('renderItem')) {
            const _origSmallRender = UTSmallPlayerItemView.prototype.renderItem;
            UTSmallPlayerItemView.prototype.renderItem = function(p, t) {
                _origSmallRender.call(this, p, t);
                if (p && p.type === ItemType.PLAYER && !services.PlayerMetaData.metaDAO.metaRepo.has(p.definitionId)) {
                    services.PlayerMetaData.updateItemPlayerMeta([p]).observe(cntlr.current(), function(t, e) {
                        t.unobserve(cntlr.current());
                    });
                ...
                if(_.size(info.base.fastsbc) > 0 && this._fsu?.quickOther){
                    ...
                    if(q){
                        ...
                        if(qs){
                            ...
                            //26.10-jacyi.8 [C-06] 一键完成/AUTO SOLVE 已并入智能填充（阵容页唯一填充入口），此栏保留流程按钮
                            ...
                            if (isDaily && this._challenge.repeats) {
                                let remaining = Math.max(0, this._challenge.repeats - this._challenge.timesCompleted);
                                if (remaining >= 1) {
                                    var _bBtn = events.createButton(
                                        new UTButtonControl(),
                                        fy(["fastsbc.clearbtn", remaining]),
                                        () => {
                                            if (info.base.fastsbctips) {
                                                info.run._lastAction = "batch";
                                                info.run._fastBatchInfo = { total: remaining, current: 0 };
                                ...
                                //26.10-jacyi.8 [C-06] quicklyBtn 已并入智能填充 → 批量按钮改挂 quickOther 末尾
                                this._fsu?.quickOther?.append(_bBtn.getRootElement());
                                }
                            } else {
        ...
        events.sbcInfoFill = (d,e) => {
            //26.09-mod-05 不再早期返回，新SBC无数据时优雅降级（仍显示价格等信息）
            let s = info.task.sbc.stat?.[d];
            if(_.has()){
            ...
            if(!e.data.isComplete()){
                ...
                if(_.size(fastInfo)){
                    if(e.data.challengesCount == 1){
                        ...
                        //batch btn / 每日任务一键清空
                        let setObj = services.SBC.repository.getSetById(e.data.id);
                        let challenge = setObj ? setObj.getChallenge(fastCid) : null;
                        let sbcName = challenge ? challenge.name : '';
                        let isDaily = sbcName.indexOf('Daily') === 0 || sbcName.indexOf('每日') === 0;
                        if (isDaily && challenge && challenge.repeats) {
                            let remaining = Math.max(0, challenge.repeats - challenge.timesCompleted);
                            if (remaining >= 1) {
            ...
            //26.09-mod-05 新SBC无数据时显示"暂无数据"，不崩溃
            if(e._interactionState && !e.__root.querySelector(".fsu-sbc-info")){
                let p = s ? s[info.base.platform] : 0;
                let up = s ? `${s.u}%` : '--';
                let down = s ? `${s.d}%` : '--';
                e.__root.lastChild.before(events.createDF(fy(html.sbcInfo).replace("{price}",Number(p).toLocaleString()).replace("{up}",up).replace("{down}",down)))
            }
                    ...
                    for (let i of e._challengeRows) {
                        if(i?._fsu?.subSet){
                            ...
                            if(fast){
                                ...
                                //batch btn / 每日任务一键清空
                                let subName = i._fsu.subSet.name || '';
                                let isDaily = subName.indexOf('Daily') === 0 || subName.indexOf('每日') === 0;
                                if (isDaily && i._fsu.subSet.repeats) {
                                    let remaining = Math.max(0, i._fsu.subSet.repeats - i._fsu.subSet.timesCompleted);
                                    if (remaining >= 1) {
                                        var _bBtn = events.createButton(
                                            new UTCurrencyButtonControl(),
                                            fy(["fastsbc.clearbtn", remaining]),
                ...
                if(controller instanceof UTSlotDetailsViewController ){
                    ...
                    if(controller.squad.isSBC()){
                        ...
                        if(controller.squad._fsu.hasChemistry){
                            ...
                            //26.09-mod-03 移除 meetsRequirements() 门控，始终显示"满足需求球员"按钮
                            //原条件 controller.challenge.meetsRequirements() 导致阵容不达标时按钮不显示
        ...
        fsuSV.prototype._generate = function _generate() {
            if (!this._generated) {
                ...
                //26.10-jacyi.8 [C-06] 智能填充默认选项区块（填充菜单预载值；0 = 不限；AUTO SOLVE 已并入智能填充）
                let fillBox = document.createElement("div");
                fillBox.classList.add("sort-filter-container");
                let fillTitle = document.createElement("h4");
                fillTitle.textContent = fy("set.sbcFill.title");
                fillBox.appendChild(fillTitle);
                let fillInner = document.createElement("div");
                fillInner.classList.add("fsu-setbox");
                fillInner.appendChild(set.addSelect("sbcFill","poolSource",
                    [["unassigned", fy("smartfill.pool.unassigned")], ["storage", fy("smartfill.pool.storage")]],
                    info.set.sbcFill_poolSource === "storage" ? "storage" : "unassigned"));
                fillInner.appendChild(set.addNumber("sbcFill","minRating", info.set.sbcFill_minRating || 0, 0, 99, 1));
                fillInner.appendChild(set.addNumber("sbcFill","maxRating", info.set.sbcFill_maxRating || 0, 0, 99, 1));
                fillInner.appendChild(set.addNumber("sbcFill","minPrice", info.set.sbcFill_minPrice || 0, 0, 9999999, 100));
                fillInner.appendChild(set.addNumber("sbcFill","maxPrice", info.set.sbcFill_maxPrice || 0, 0, 9999999, 100));
                fillInner.appendChild(set.addToggle("sbcFill","preferDuplicates"));
                fillInner.appendChild(set.addToggle("sbcFill","untradeableOnly"));
                fillInner.appendChild(set.addToggle("sbcFill","commonsOnly"));
                fillInner.appendChild(set.addToggle("sbcFill","useTemplate"));
                fillBox.appendChild(fillInner);
                i.appendChild(fillBox);
        ...
        events.playerListFillSquad = (challenge,list,type) => {
            ...
            playerlist = squadBestPos.map(function(e, t) {
                ...
                if(!i || i.isBrick()){
                    ...
                    //26.09-mod-02 去掉 type==2 快捷条件，让一键填充/阵容补全都受 ignorepos 控制
                        ...
                        if(packInfo){
                            ...
                            if ((packInfo.isPlayers || packInfo.tradable) && !itemElement.querySelector(".fsu-bulkopen")) {
                                ...
                                let bulkOpenBtn = events.createButton(
                                    ...
                                    (e) => {
                                        //26.10-jacyi.1 [C-01] 数量弹窗 + 26.10-jacyi.9 [C-09] 模式三选（tradable 决定红色规则）
                                        events.openPacksConfirmPopup(item.articleId, packInfo.fullName, packInfo.count, false, undefined, packInfo.tradable);
                                    },
                                    "fsu-bulkopen call-to-action"
                                )
                                bulkOpenBtn.__currencyLabel.textContent = fy("openpack.storebtn.subtext")

                                //26.10-jacyi.9 [C-09] 按钮精简：只留 批量开包 + 原生打开扩展包（各 1/2）
                                //模式选择（普通/出售重复/全部出售）收进确认弹窗；红色规则按包可交易性（弹窗内处理）
                                ...
                                for (let i = 0; i < container.children.length; i++) {
                                    ...
                                    if (!child.classList.contains("fsu-bulkopen")) {
                                        child.style.flex = "1";
        ...
        UTPackAnimationViewController.prototype.runAnimation = function() {
            if (!this.running) {
                ...
                //26.09-mod-01 开包时加载球员元数据，确保评分正确显示
                if(this.presentedItem && this.presentedItem.type === ItemType.PLAYER){
                    services.PlayerMetaData.updateItemPlayerMeta([this.presentedItem]).observe(cntlr.current(), function(t, e) {
                        t.unobserve(cntlr.current());
                    });
                }
        ...
        events.oneFillCreationGF = (req,miss) => {
            ...
            _.map(req,i => {
                ...
                switch (fk) {
                    ...
                    case SBCEligibilityKey.PLAYER_RARITY_GROUP:
                        ...
                        break;
                    //26.09-mod-06 放宽条件：只要有 PLAYER_MIN_OVR 就加入，不要求是唯一条件
                    case SBCEligibilityKey.PLAYER_MIN_OVR:
                        gfs.t["GTrating"] = fv;
                        break;
                    default:
            ...
            //26.09-mod-06 仅当没有其他填充条件时才清空 gf（TEAM_RATING/CHEMISTRY 可基于球员自动计算）
            if (gf.length === 0) {
                _.map(req,r => {
        ...
        UTSBCService.prototype.submitChallenge = function(o, a, i, n) {
            ...
            r.observe(this, function(e,t) {
                ...
                if(t.success){
                    ...
                    //26.09-mod-08 记录每日SBC完成顺序
                    let sbcName = "Unknown";
                    try {
                        if (a && a.name) sbcName = a.name;
                        else if (t.data && t.data.setId) {
                            let setEntity = services.SBC.repository.getSetById(t.data.setId);
                            if (setEntity && setEntity.name) sbcName = setEntity.name;
                        }
                    } catch(e) {}
                    let dailyLog = JSON.parse(GM_getValue("SBCDailyLog", "[]"));
                    if (dailyLog.length > 0) {
                        let firstDate = new Date(dailyLog[0].ts);
                        firstDate.setHours(0,0,0,0);
                        if (firstDate.getTime() !== DT) dailyLog = [];
                    }
                    dailyLog.push({
                        name: sbcName,
        ...
        SBCCount.createElement = (ne) => {
            ...
            if(!_.has(info.nave,"SBCCount")){
                info.nave.SBCCount = events.createButton(
                    ...
                    async(e) => {
                        let sbcLog = JSON.parse(GM_getValue("SBCDailyLog", "[]"));
                        let popupHtml = fy("sbccount.popupm");
                        if (sbcLog.length > 0) {
                            let listHtml = sbcLog.map((entry, i) =>
                                (i+1) + ". [" + entry.time + "] " + entry.name
                            ).join("<br>");
                            popupHtml += "<br><br><b>今日完成顺序：</b><br>" + listHtml;
                        }
                        events.popup(
                            fy("sbccount.popupt"),
                            popupHtml,
        ...
        //** 25.21 批量开包：开包确认弹窗 */
        events.openPacksConfirmPopup = (packId, packName, packCount, sellDup, mode, tradable) => {
            //26.10-jacyi.9 [C-09] 包可交易性（决定出售类模式的红色警示；默认 true 兼容旧调用）
            ...
            //26.10-jacyi.9 [C-09] 红色规则按包可交易性：不可交易包选出售重复/全部出售均红；可交易包仅全部出售红
            ...
            const modeBtns = {};
            //26.10-jacyi.9 [C-09] 不可交易包警示行（选出售类时显示）
            const warnLine = document.createElement("div");
            warnLine.textContent = fy("openpack.popup.untradable.warn");
            warnLine.style.cssText = "color:#c0392b;text-align:center;margin:.4rem auto 0;font-size:.9rem;display:none;";
            const applyMode = (m) => {
                currentMode = m;
                ...
                const bb = modeBtns[m];
                const danger = m === 'sellAll' || (m === 'sellDup' && !tradable);
                bb.style.borderColor = danger ? "#c0392b" : "#2196F3";
                bb.style.color = danger ? "#fff" : "#2196F3";
                bb.style.background = danger ? "#c0392b" : "#333";
                warnLine.style.display = (!tradable && m !== 'normal') ? "block" : "none";
            };
            ...
            popupView.__msg.appendChild(warnLine)
        ...
        UTSBCRequirementsView.prototype.render = function(e, t, i, o) {
            ...
            if(_.has(info.base.fastsbc,sName)){
                ...
                let sbcName = e.name || '';
                let isDaily = sbcName.indexOf('Daily') === 0 || sbcName.indexOf('每日') === 0;
                if (isDaily && e.repeats) {
                    let remaining = Math.max(0, e.repeats - e.timesCompleted);
                    if (remaining >= 1) {
                        var _bBtn = events.createButton(
                            new UTCurrencyButtonControl(),
                            fy(["fastsbc.clearbtn", remaining]),
        ...
        UTSBCSquadOverviewViewController.prototype.initWithSBCSet = function (...args) {
            ...
            if(this._challenge){
                ...
                if(oneFillCriteria.length){
                    ...
                    //26.10-jacyi.8 [C-06] 智能填充单按钮：合并一键填充/重复填充/阵容补全/方案填充/一键完成/AUTO SOLVE
                    if(resolveSmartFillEnabled(info.set)){
                        this._fsu.smartFillBtn = events.createButton(
                            new UTStandardButtonControl(),
                            fy("smartfill.btntext"),
                            async () => { await events.smartFill.runForCurrent(thisController); },
                            "call-to-action"
                        )
                        this._fsu.smartFillBtn.challenge = this._challenge;
                    }
            }
        }
        ...
        UTSBCSquadDetailPanelViewController.prototype.initWithSBCSet = function(...args) {
            ...
            //26.10-jacyi.8 [C-06] 智能填充按钮（唯一填充入口；无化学/需求闸门）
            if(resolveSmartFillEnabled(info.set) && fsu?.smartFillBtn){
                exchangeElement.before(fsu.smartFillBtn.getRootElement())
            }

            //[C-06] 布局：智能填充按钮存在即执行 mini 化（化学 SBC 页同样需要，避免按钮错位）
            if(fsu?.smartFillBtn){
        ...
        //26.10-jacyi.9 [C-01] EA observable 超时（修复开包挂起：兑换界面/controller 切换时观察回调不触发）
        // 超时后不重试（防双开），物品留 EA 未分配区由下次批量前置检查兜住
        var PACK_OBSERVE_TIMEOUT_MS = 60000;
        ...
        // 26.10-jacyi.9 [C-01] 五去向：+ toRedeem（金币/内层包等 misc 条目，照 fodder：
        //   type === ItemType.MISC → 兑换；sellAll 下不卖 misc）；picks（球员选择卡）→ toUnassigned
        ...
        // 26.10-jacyi.9 新格式：开包 3/10 ｜ 发送俱乐部 12 个 ｜ 放入SBC仓库 8 个 ｜ 出售物品 5 个（金币 3,200） ｜ 兑换 2 个
        ...
        packs._makeDeps = () => {
            const controller = cntlr.current();
            //26.10-jacyi.9 [C-01] observable → promise：60s 超时 + observe 异常兜底（不再永久挂起）
            const observeOnce = (obs) => new Promise((resolve) => {
                let done = false;
                const finish = (t) => { if (done) return; done = true; resolve(t); };
                try {
                    obs.observe(controller, (e, t) => {
                        try { e.unobserve(controller); } catch (e2) {}
                        finish(t);
                    });
                } catch (e) {
                    // controller 已切换等 → 立即失败，走重试/降级（修复停止按钮"失效"根因）
                    finish({ success: false, error: { code: "observe-error" } });
                    return;
                }
                setTimeout(() => finish({ success: false, error: { code: "timeout" } }), PACK_OBSERVE_TIMEOUT_MS);
            });
            const isCancelled = () => !!(packs._cancelToken && packs._cancelToken.cancelled);
            // 可中断 sleep：200ms 轮询取消令牌，取消提前返回 true
            const sleep = (ms) => new Promise((resolve) => {
                const until = Date.now() + ms;
                const tick = () => {
                    if (isCancelled()) { resolve(true); return; }
            ...
            return {
                ...
                //26.10-jacyi.9 [C-01] 兑换（照 fodder：services.Item.redeem 单条目，observable/promise 双形态）
                redeem: (item) => {
                    try {
                        const fn = services.Item && typeof services.Item.redeem === "function" ? services.Item.redeem : null;
                        if (!fn) return Promise.resolve({ success: false, error: { code: "no-redeem" } });
                        const r = fn.call(services.Item, item);
                        if (r && typeof r.observe === "function") return observeOnce(r);
                        return Promise.resolve(r);
                    } catch (e) { return Promise.resolve({ success: false, error: { code: "redeem-error" } }); }
                },
                isCancelled: isCancelled,
                sleep: sleep,
                isPick: (item) => {
                    try { return typeof item.isPlayerPickItem === "function" && !!item.isPlayerPickItem(); }
                    catch (e) { return false; }
                },
                isRedeemable: (item) => {
                    try { return item.type === ((window.ItemType && window.ItemType.MISC) || "misc"); }
                    catch (e) { return false; }
                },
        ...
        // 单包处理：开 → 分类 → 移入/出售/兑换（降级矩阵）→ 记录
        // 返回 { opened:bool, failed:bool, fatal:bool, stop:bool, cancelled:bool }
        packs.openOne = async (pack, index, options, deps, records) => {
            //26.10-jacyi.9 [C-01] 入口取消检查
            ...
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    //26.10-jacyi.9 [C-01] 超时/观察异常不重试（防双开），物品留未分配由下次批量前置检查兜住
            ...
            const ctx = {
                ...
                //26.10-jacyi.9 [C-01] 五去向：picks/misc 分流判定
                isPick: deps.isPick,
            ...
            // 1) 新球员 → 俱乐部（失败：留未分配，但不再提前 return —— 26.10-jacyi.9 修复记录丢失）
            ...
            // 4) 兑换桶（26.10-jacyi.9 [C-01] 照 fodder：金币/内层包等 misc 逐条 services.Item.redeem，
            //    成功 storeLoc=4，失败留未分配不中断批）
            ...
            // 5) 未分配兜底（overfillMode=unassigned）：物品自然留在未分配，只记录
            //26.10-jacyi.9 [C-01] 无法分类条目（无 type 字段）输出结构供实测反馈
        ...
        packs.open = async (options, deps) => {
            ...
            const progress = { done: 0, clubCount: 0, storageCount: 0, sellCount: 0, sellCoins: 0, redeemCount: 0, unassignedCount: 0 };

            //26.10-jacyi.9 [C-01] 进度文案新键（发送俱乐部/放入SBC仓库/出售物品/兑换，{count} 占位模板）
            const texts = {
                prefix: deps.fy(["openpack.progress.bottombar", "{done}", "{total}"]),
                stop: deps.fy("openpack.progress.stop"),
                toclub: deps.fy("openpack.progress.toclub"),
                tostorage: deps.fy("openpack.progress.tostorage"),
                tosell: deps.fy("openpack.progress.tosell"),
                redeem: deps.fy("openpack.progress.redeem"),
            ...
            try {
                for (let i = 0; i < targetPacks.length; i++) {
                    ...
                    if (r.stop) break;
                    //26.10-jacyi.9 [C-01] openOne 内部取消（包中途点停止）
                    ...
                    progress.redeemCount = s.redeemCount;
                    progress.unassignedCount = s.unassignedCount;
                    bar.querySelector("#fsu-packprogress-text").textContent = formatProgressText(progress, targetPacks.length, texts);

                    if (i < targetPacks.length - 1) {
                        //26.10-jacyi.9 [C-01] 可中断 sleep：取消提前返回
                        if (await deps.sleep(500 + Math.floor(Math.random() * 1000))) { cancelled = true; break; }
                    }
                }
            } finally {
                ...
                const popupText = deps.fy(["openpack.result.popupm3",
                    ...
                    stats.specialCount, stats.playerMaxRating, stats.unassignedCount, stats.redeemCount]);
        ...
        //PURE: EA 需求对象数组 → 简化形 [{key,value,count}]（与 fill.run 旧内联循环逐行等价）
        ...
        //PURE: 智能填充分流判定："basic"（全部键 ∈ 基础集：品质/稀有度/TOTW/最低评分/阵容总评）→ 本地算法快速填充；
        // "complex"（含化学/俱乐部/联赛/国籍/精确评分/位置/未知键）→ Futbin 模板优先
        // TEAM_RATING（均分）属 basic：本地引擎 ratingCombo 可直接求解，无需外部模板
        ...
        //PURE: 填充选项规范化（smartfill 菜单/per-SBC 保存共用）
        // 0/空 = 不限(null)；评分钳 40-99、价格 ≥0；min>max 自动交换；布尔强转；缺省回退 defaults
        ...
        //PURE: 智能填充开关兼容解析（sbc_smartfill 显式优先；未定义 → 默认开，存量旧开关功能超集覆盖）
        ...
        fill.run = async (challenge, opts, deps) => {
            ...
            // EA 需求对象 → 简化形（simplifyReqs 与旧内联循环逐行等价）
            ...
            // [C-05] smartFill 选项：池预过滤（filterCandidates：评分/价格/普通卡/不可交易）+ 数据源 + 优先重复
            let pool = deps.pool();
            ...
            if (o.poolSource === "storage") {
                // 数据源切换：池传 undefined → events.getItemBy 自动用 俱乐部+仓库（replaceData 空值语义 L841）
                pool = null;
                const orig = getItemBy;
                getItemBy = (crit, p, rd) => orig(crit, p == null ? undefined : p, rd);
            }
            if (o.preferDuplicates) {
                // 优先重复：稳定排序（重复球员在前），非过滤；getItemBy 每次返回新数组无缓存污染
                const orig = getItemBy;
                getItemBy = (crit, p, rd) => {
            ...
            // storage 模式 pool 为 null：池规模判定跳过（club+storage 默认全量，不误报 fill.error.pool）
        ...
        // ============================================================
        // [C-06] 智能填充单按钮（v26.10-jacyi.8）：填充 + 自动提交一体
        // ------------------------------------------------------------
        // 点击 → 填充选项菜单（per-SBC 记忆，GM key "SBCFillOptions"）→ 分流：
        //   basic（品质/稀有度/TOTW/最低评分）→ 本地引擎（quickGreedy 链）
        //   complex（总评/化学/俱乐部/联赛/国籍/精确评分/位置）→ Futbin 模板优先 → 引擎兜底
        // 成功 → 落阵 → submitChallengeFlow 自动提交；失败 → classifyFailure 分类通知
        // 契约：getTemplate 成功时 push 到 ch.squad._fsu.templatePlan（L9085），以此判定成败
        // ============================================================
        events.smartFill = {};

        // 全局默认（设置面板 sbcFill_* 可覆盖；0 = 不限）
        events.smartFill.defaults = () => normalizeFillOptions(null, {
            poolSource: (info.set && info.set.sbcFill_poolSource) || "unassigned",
            untradeableOnly: !!(info.set && info.set.sbcFill_untradeableOnly),
            preferDuplicates: info.set && info.set.sbcFill_preferDuplicates !== undefined ? !!info.set.sbcFill_preferDuplicates : true,
            commonsOnly: !!(info.set && info.set.sbcFill_commonsOnly),
            useTemplate: info.set && info.set.sbcFill_useTemplate !== undefined ? !!info.set.sbcFill_useTemplate : true,
            minRating: (info.set && info.set.sbcFill_minRating) || null,
            maxRating: (info.set && info.set.sbcFill_maxRating) || null,
            minPrice: (info.set && info.set.sbcFill_minPrice) || null,
            maxPrice: (info.set && info.set.sbcFill_maxPrice) || null
        });

        events.smartFill._loadStore = () => {
            try { return JSON.parse(GM_getValue("SBCFillOptions", "{}")) || {}; }
            catch (e) { return {}; }
        };
        events.smartFill._saveStore = (store) => {
            try { GM_setValue("SBCFillOptions", JSON.stringify(store)); }
            catch (e) { console.warn("[C-06] SBCFillOptions 保存失败", e); }
        };

        // 填充选项菜单（events.popup + HTML 表单；确认 → {opts, templateRef, remember}，取消 → null）
        events.smartFill.showMenu = (opts) => new Promise((resolve) => {
            const o = opts || events.smartFill.defaults();
            const chk = (id, cur, label) =>
                `<div style="display:flex;align-items:center;justify-content:space-between;gap:.5rem">
                    <span style="color:#ccc">${label}</span>
                    <input id="${id}" type="checkbox"${cur ? " checked" : ""} style="width:1rem;height:1rem">
                </div>`;
            const num = (id, cur) =>
                `<input id="${id}" type="number" min="0" value="${cur != null ? cur : "0"}" style="background:#222;color:#ccc;border:1px solid #555;border-radius:4px;padding:.2rem .4rem;width:4.5rem">`;
            const rangeRow = (label, minId, maxId, minCur, maxCur) =>
                `<div style="display:flex;align-items:center;justify-content:space-between;gap:.5rem">
                    <span style="color:#ccc">${label}</span>
                    <span style="display:flex;align-items:center;gap:.3rem">${num(minId, minCur)}<span style="color:#888">~</span>${num(maxId, maxCur)}</span>
                </div>`;
            const selRow = `<div style="display:flex;align-items:center;justify-content:space-between;gap:.5rem">
                    <span style="color:#ccc">${fy("smartfill.pool")}</span>
                    <select id="fsu-smartfill-pool" style="background:#222;color:#ccc;border:1px solid #555;border-radius:4px;padding:.2rem .4rem">
                        <option value="unassigned"${o.poolSource !== "storage" ? " selected" : ""}>${fy("smartfill.pool.unassigned")}</option>
                        <option value="storage"${o.poolSource === "storage" ? " selected" : ""}>${fy("smartfill.pool.storage")}</option>
                    </select>
                </div>`;
            const refRow = `<input id="fsu-smartfill-ref" type="text" placeholder="${fy("smartfill.option.ref")}" style="background:#222;color:#ccc;border:1px solid #555;border-radius:4px;padding:.2rem .4rem;width:100%">`;
            const html =
                `<div id="fsu-smartfill" style="display:flex;flex-direction:column;gap:.45rem">
                    ${selRow}
                    ${rangeRow(fy("smartfill.ratingrange"), "fsu-smartfill-minrating", "fsu-smartfill-maxrating", o.minRating, o.maxRating)}
                    ${rangeRow(fy("smartfill.pricerange"), "fsu-smartfill-minprice", "fsu-smartfill-maxprice", o.minPrice, o.maxPrice)}
                    ${chk("fsu-smartfill-untradeable", o.untradeableOnly, fy("smartfill.option.untradeable"))}
                    ${chk("fsu-smartfill-dup", o.preferDuplicates, fy("smartfill.option.dup"))}
                    ${chk("fsu-smartfill-commons", o.commonsOnly, fy("smartfill.option.commons"))}
                    ${chk("fsu-smartfill-template", o.useTemplate, fy("smartfill.option.template"))}
                    ${refRow}
                    ${chk("fsu-smartfill-remember", true, fy("smartfill.option.remember"))}
                </div>`;
            events.popup(fy("smartfill.popupt"), html, (z) => {
                if (z !== 2) { resolve(null); return; }
                const $ = (id) => document.getElementById(id);
                const opts = normalizeFillOptions({
                    poolSource: $("fsu-smartfill-pool").value,
                    untradeableOnly: $("fsu-smartfill-untradeable").checked,
                    preferDuplicates: $("fsu-smartfill-dup").checked,
                    commonsOnly: $("fsu-smartfill-commons").checked,
                    useTemplate: $("fsu-smartfill-template").checked,
                    minRating: $("fsu-smartfill-minrating").value,
                    maxRating: $("fsu-smartfill-maxrating").value,
                    minPrice: $("fsu-smartfill-minprice").value,
                    maxPrice: $("fsu-smartfill-maxprice").value
                }, events.smartFill.defaults());
                resolve({ opts: opts, templateRef: ($("fsu-smartfill-ref").value || "").trim(), remember: $("fsu-smartfill-remember").checked });
            });
        });

        // 主流程：菜单 → 分流（basic 引擎 / complex 模板优先→引擎兜底）→ 落阵 → 自动提交
        events.smartFill.runForCurrent = async (controller) => {
            const ch = controller && (controller._challenge || (controller.viewmodel && controller.viewmodel._challenge));
            if (!ch) { events.notice(fy("smartfill.btntext"), 2); return; }
            const key = `${ch.id}#${ch.setId}`;
            const store = events.smartFill._loadStore();
            const preload = loadFillOptions(key, store, events.smartFill.defaults());
            const menu = await events.smartFill.showMenu(preload);
            if (!menu) return; // 取消
            if (menu.remember) events.smartFill._saveStore(saveFillOptions(store, key, menu.opts));
            const opts = menu.opts;
            const deps = fill._makeDeps();
            events.showLoader();
            try {
                const mode = classifySbcMode(simplifyReqs(ch.eligibilityRequirements));
                const templateCount = () => (ch.squad && ch.squad._fsu && ch.squad._fsu.templatePlan) ? ch.squad._fsu.templatePlan.length : 0;
                const applyTemplate = async (type, sId) => {
                    const before = templateCount();
                    try { await events.getTemplate({ challenge: ch, setInteractionState: () => {} }, type, sId); }
                    catch (e) { console.warn("[C-06] getTemplate 异常", e); }
                    return templateCount() > before;
                };
                let usedTemplate = false;
                // 方案链接/ID 显式指定 → 直接模板（跳过分流）
                const mGG = menu.templateRef ? menu.templateRef.match(/(?:squad-builder\/|^)([a-f0-9-]{36})/) : null;
                const mBin = menu.templateRef ? menu.templateRef.match(/(?:squad\/|^)(\d+)/) : null;
                if (menu.templateRef) {
                    if (!mGG && !mBin) { events.notice("consult.error", 2); return; }
                    usedTemplate = await applyTemplate(mGG ? 3 : 2, mGG ? mGG[1] : mBin[1]);
                } else if (mode === "complex" && opts.useTemplate) {
                    usedTemplate = await applyTemplate(1);
                }
                if (usedTemplate) {
                    events.notice(fy("smartfill.template.ok"), 0);
                } else {
                    // 本地引擎（basic 直接引擎；complex 模板失败兜底）
                    if (mode === "complex" && opts.useTemplate) events.notice(fy("smartfill.template.fail"), 1);
                    const r = await fill.run(ch, Object.assign({ algorithm: "auto" }, opts), deps);
                    if (!r.ok || !r.filled.length) {
                        deps.notify(deps.fy(r.reason || "fill.error.noanswer"), 2);
                        return;
                    }
                    try { deps.playerListFillSquad(ch, r.filled, 1); } catch (e) { console.warn("[C-06] 落阵失败", e); }
                    events.notice(fy("fill.done"), 0);
                }
                // 提交（模板路径内部已 saveSquad，无需重复落阵）
                const setEntity = services.SBC.repository.getSetById(ch.setId);
                await events.submitChallengeFlow(ch, setEntity, controller);
            } finally {
                events.hideLoader();
            }
        };
