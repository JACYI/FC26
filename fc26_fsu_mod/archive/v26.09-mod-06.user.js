        ...
        events.reloadPlayers = async(retryCount) =>{
            ...
            await services.Club.getStats().observe(current,async function _onGetStats(e, t) {
                ...
                t.success ? t.response.stats.forEach(async function(e) {
                    ...
                    //26.09-mod-04 失败时带退避重试，连续失败3次后才提示用户
                    var currentRetry = (typeof retryCount === 'number' ? retryCount : 0);
                    events._reloadRetry = currentRetry + 1;
                    if (events._reloadRetry <= 3) {
                        console.warn("[FSU] Player data load failed, retry " + events._reloadRetry + "/3 in " + (events._reloadRetry * 2) + "s");
                        setTimeout(function() {
                            events.reloadPlayers(events._reloadRetry);
                        }, events._reloadRetry * 2000);
                    } else {
                        NetworkErrorManager.checkCriticalStatus(response.status) && NetworkErrorManager.handleStatus(response.status);
                        events.hideLoader();
                        events.notice("notice.ldataerror",2);
                    }
                }(events._reloadRetry);
            });
            //26.09-mod-04 成功加载时在 notice.ldatasuccess 前重置 retry 计数器
        }
