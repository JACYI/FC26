...
(function () {
    ...
    function futweb() {
        ...
        events.reloadPlayers = async(retryCount) =>{
            ...
            //26.09-mod-07 优先级分层加载+SBC仓库
            const pageSize = info.set.players_page_size || 500;
            const pageDelay = info.set.players_page_delay || 0.3;
            const priorityRanges = [{min:40,max:60},{min:65,max:70},{min:75,max:84}];
            const bgRanges = [{min:61,max:64},{min:71,max:74},{min:85,max:99}];

            let loadRange = async (ovrMin, ovrMax, showLdr) => {
                if (showLdr) events.showLoader();
                let page = 0;
                while (true) {
                    let c = new UTSearchCriteriaDTO();
                    c.type = "player"; c.sortBy = "ovr"; c.sort = "desc";
                    c.count = pageSize; c.offset = page * pageSize;
                    c.ovrMin = ovrMin; c.ovrMax = ovrMax;
                    if (showLdr) events.changeLoadingText(["loadingclose.ldata", page + '', '?']);
                    try {
                        let r = await new Promise((rs, rj) => {
                            services.Club.search(c).observe(getAppMain().getRootViewController(), (p, t) => {
                                if (p.unobserve(p), t.success && JSUtils.isObject(t.response)) rs(t.response);
                                else rj(new Error("fail"));
                            });
                        });
                        let items = r.items || [];
                        ...
                        await events.wait(pageDelay * 0.5, pageDelay * 1.5);
                    } catch (e) { console.error("Search error:", e); break; }
                }
                if (showLdr) events.hideLoader();
            };

            //优先层（阻塞）
            for (const r of priorityRanges) await loadRange(r.min, r.max, true);
            info.base.state = true;
            events._reloadRetry = 0;
            events.notice("notice.ldatasuccess",0);
            //后台层（不阻塞）
            for (const r of bgRanges) loadRange(r.min, r.max, false);
            //加载SBC仓库
            services.Item.searchStorageItems(new UTSearchCriteriaDTO()).observe(getAppMain().getRootViewController(), function(e, t) {
                e.unobserve(getAppMain().getRootViewController());
            });
        }
