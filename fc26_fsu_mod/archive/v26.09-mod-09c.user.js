...
(function () {
    ...
    function futweb() {
        ...
        info.localization = {
            ...
            "squadcmpl.defaultbtn":["默认填充评分","默認填充評分","Default fill ratings"],
            "squadcmpl.none":["（无）","（無）","(none)"],
            "squadcmpl.defaultpopupm":["输入格式：目标评分:评分（用英文逗号分隔）。例如「83:84,84,84,84,84,84,84,84,84,83,83」。清空输入框并确定可清除所有配置。","輸入格式：目標評分:評分（用英文逗號分隔）。例如「83:84,84,84,84,84,84,84,84,84,83,83」。清空輸入框並確定可清除所有配置。","Format: targetRating:ratings (comma separated). E.g. '83:84,84,84,84,84,84,84,84,84,83,83'. Clear input and confirm to reset all."],
        ...
        fsuSV.prototype._generate = function _generate() {
            if (!this._generated) {
                ...
                //阵容补全默认评分
                this._fsusquadcmpl = events.createButton(
                    new UTStandardButtonControl(),
                    fy("squadcmpl.defaultbtn"),
                    () => {
                        //显示当前已保存的配置
                        let saved = info.set.squadcmpl_default || {};
                        let summary = Object.keys(saved).length ?
                            Object.entries(saved).map(([k,v]) => `${k}: ${v}`).join("\n") :
                            fy("squadcmpl.none");
                        events.popup(
                            fy("squadcmpl.defaultbtn"),
                            fy("squadcmpl.defaultpopupm") + "\n\n" + summary,
                            (t,i) => {
                                if(t === 2){
                                    let v = i.getValue().trim();
                                    if(v === ""){
                                        set.save("squadcmpl_default",{});
                                        events.notice(fy("notice.setsuccess"),0);
                                    }else{
                                        //格式: 83:84,84,84,84,84,84,84,84,84,83,83
                                        let m = v.match(/^(\d{2}):(.+)$/);
                                        if(m){
                                            let target = m[1];
                                            let ratings = m[2].split(",").map(Number);
                                            if(ratings.every(r => !isNaN(r) && r >= 40 && r <= 99)){
                                                let all = Object.assign({}, info.set.squadcmpl_default || {});
                                                all[target] = m[2];
                                                set.save("squadcmpl_default", all);
                                                events.notice(fy("notice.setsuccess"),0);
                                            }else{
                ...
                b.appendChild(this._fsusquadcmpl.__root);
        ...
        set.init = function(){
            ...
            //26.09-mod-07 球员加载配置
            if(!b.hasOwnProperty("players_page_size")){
                b["players_page_size"] = 500;
            }
            if(!b.hasOwnProperty("players_page_delay")){
                b["players_page_delay"] = 0.3;
            }
            console.log(b)
            info.set = b;
        }
        ...
        events.goToStoragePlayers = (e) => {
            ...
            if(nav){
                ...
                criteria.sortBy = SearchSortType.RATING;
                criteria.sort = "asc";
        ...
        UTClubSearchResultsViewController.prototype._requestItems = function(r) {
            if("_fsuLock" in this && this._fsuLock){
                ...
                var e = this.getView().getSubTypesDropDown()
                    , t = new UTSearchCriteriaDTO;
                t.update(this.searchCriteria),
                t.sortBy = SearchSortType.RATING,
                t.sort = "asc",
                0 < e.length && (t.subtypes = [e.id]),
                    ...
                    this._fsu.squadCmplBtn = events.createButton(
                        ...
                        (e) => {
                            ...
                            if(fillRating.length && fillRating[0].lackRatings.length == 0 && fillRating[0].ratings.length && hasRating){
                                let savedDefaults = info.set.squadcmpl_default || {};
                                let customDefault = savedDefaults[String(hasRating)];
                                let defaultRatings = customDefault || fillRating[0].ratings.join(`,`);
                                inputText = [fy("squadcmpl.placeholder"), fillRating.length == "0" && va == 0 ? "" : defaultRatings];
                            }
